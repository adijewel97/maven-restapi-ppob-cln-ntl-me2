...
@Path("/ppobntl")
public class Ntl_01CPaymentController_22_me1 {
    ...
    public String addBanklogin(String jsonBody) {
        ...
        for (JsonElement element : inputArray) {
            ...
            message = GetJsonToMsg_2200(jsonBody);
            if (message.isEmpty()) {
            ...
            try {
                serverResponseArrayParsing = JsonParser.parseString(ResponServerParsing_2210(serverResponse)).getAsJsonArray();
            } catch (Exception e) {
                vstatus = "ERROR";
    ...
    private String createSuccessResponse(String status, String message, String clientMessage, String serverResponse, JsonArray serverResponseParsing) {
        ...
        response.add("ClientInqueryParsing", JsonParser.parseString(ReguestClientParsing_2200(clientMessage)));
    ...
    public String GetJsonToMsg_2200(String jsonBody) {
        ...
        for (JsonElement element : inputArray) {
            ...
            if (typeMessage == 1) {
                if (!inputObject.has("message") || inputObject.get("message").getAsString().isEmpty()) {
                    ...
                    String formattedJsonOutput = ReguestClientParsing_2200(message);
                ...
                String refnum_srv                    = inputObject.has("refnum_srv")                    ? inputObject.get("refnum_srv").getAsString() : "";
                ...
                sb.append(mti).append(bit_map).append(pan_len).append(pan)
                .append(CurrISOCode).append(CurrMinorUnit).append(CurrValueAmount)
                .append(switcher_tan).append(dtl_trans).append(merchant)
                .append(bank_len).append(bank_id).append(apd_Len)
                .append(switcher_id).append(register_num).append(trans_code)
                .append(String.format("%-25s", trans_name)) // Padding 25 karakter
                .append(register_date).append(register_expiredate)
                .append(Subscriber_id.isEmpty() ? " ".repeat(12) : String.format("%-12s", Subscriber_id))
                .append(String.format("%-25s", Subscriber_name))
                .append(refnum_srv).append(refnum_switcher).append(service_unit)
                .append(service_unitaddress).append(service_unitphone)
                .append(trans_amount_minorunit).append(trans_amount)
                .append(servbill_minorunit)
                .append(servbill_value)
                .append(servbill_ChargeAdminMinorUnit)
                .append(servbill_ChargeAdminValue)
                .append(apd2_Len)
                .append(apd2_BillCompType)
                .append(apd2_BillCompMinorunit)
                .append(apd2_BillCompValue);
    ...
    //-----------------------------------------------------------------
    // Parsing Request Client 2200
    //-----------------------------------------------------------------
    public static String ReguestClientParsing_2200(String inputMessage) {
        ...
        try {
            ...
            String[] fields = {
                                ...
                                "switcher_tan", "dtl_trans", "merchant", "bank_len", "bank_id", "apd_Len",
                                "switcher_id", "register_num", "trans_code", "trans_name", "register_date",
                                "register_expiredate", "Subscriber_id", "Subscriber_name", "refnum_srv",
                                "refnum_switcher", "service_unit", "service_unitaddress", "service_unitphone",
                                "trans_amount_minorunit", "trans_amount", "servbill_minorunit", "servbill_value",
                                "servbill_ChargeAdminMinorUnit", "servbill_ChargeAdminValue", "apd2_Len",
                                "apd2_BillCompType", "apd2_BillCompMinorunit", "apd2_BillCompValue"
                              };
    
            // Panjang masing-masing field
            int[] fieldLengths = {
                                   4, 16, 2, 5, 3, 1, 12, 12, 14, 4, 2, 7, 3,
                                   7, 13, 3, 25, 8, 8, 12, 25, 32, 32, 5, 35,
                                   15, 1, 17, 1, 17, 1, 10, 3, 2, 1, 19
                              };
            int currentIndex = 0;
    ...
    //-----------------------------------------------------------------
    // Parsing respon server netman 2210
    //-----------------------------------------------------------------
    public static String ResponServerParsing_2210(String inputMessage) {
        JsonArray jsonArray = new JsonArray();
        JsonObject parsedObject = new JsonObject();
        ...
        try {
            ...
            String[] fields = {
                ...
                "date_settlement", "merchant", "bank_len", "bank_id", "RC", "apd_Len", "switcher_id", "register_num", "trans_code", 
                "trans_name", "register_date", "register_expiredate", "Subscriber_id", "Subscriber_name", "refnum_srv", "refnum_switcher",
                "service_unit", "v_service_unitaddress", "v_service_unitphone", "trans_amount_minorunit", "trans_amount", "servbill_minorunit",
                "servbill_value", "servbill_ChargeAdminMinorUnit", "servbill_ChargeAdminValue", "apd2_Len", "apd2_BillCompType",
                "apd2_BillCompMinorunit", "apd2_BillCompValue", "info_text_len" // Hilangkan "info_text" dari daftar awal
            };
    
            // Panjang masing-masing field
            int[] fieldLengths = {
                4, 16, 2, 5, 3, 1, 12, 12, 14, 8, 4, 2, 7, 4, 3, 7, 13, 3, 25, 8,
                8, 12, 25, 32, 32, 5, 35, 15, 1, 17, 1, 17, 1, 10, 3, 2, 1, 17, 5
            };
    
            int currentIndex = 0;
            int infoTextLength = 0; // Untuk menyimpan panjang info_text
            ...
            for (int i = 0; i < fields.length; i++) {
                if (inputMessage.length() >= currentIndex + fieldLengths[i]) {
                    ...
                    // Jika field adalah "info_text_len", simpan panjangnya
                    if (fields[i].equals("info_text_len")) {
                        infoTextLength = Integer.parseInt(value.trim()); // Konversi ke integer
                    }
            ...
            // Parsing info_text berdasarkan info_text_len
            if (infoTextLength > 0 && inputMessage.length() >= currentIndex + infoTextLength) {
                String infoText = inputMessage.substring(currentIndex, currentIndex + infoTextLength);
            
                // 1️⃣ Konversi Encoding dari Windows-1252 ke UTF-8 (jika perlu)
                byte[] bytes = infoText.getBytes(Charset.forName("Windows-1252"));
                String fixedInfoText = new String(bytes, StandardCharsets.UTF_8);
            
                // 2️⃣ Hapus karakter aneh seperti � dan ubah Non-breaking Space (U+00A0) menjadi spasi biasa
                fixedInfoText = fixedInfoText.replaceAll("[\uFFFD\u00A0]", " ");
            
                // 3️⃣ Hapus petik tunggal di awal & akhir jika ada
                if (fixedInfoText.startsWith("'") && fixedInfoText.endsWith("'")) {
                    fixedInfoText = fixedInfoText.substring(1, fixedInfoText.length() - 1);
                }
            
                // 4️⃣ Hapus backslash sebelum petik ganda agar tidak di-*escape* dalam JSON
                fixedInfoText = fixedInfoText.replace("\\\"", "\"");
            
                parsedObject.addProperty("info_text", fixedInfoText.trim());
            } else {
                parsedObject.addProperty("info_text", "");
            }
